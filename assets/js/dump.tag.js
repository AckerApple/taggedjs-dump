...
export const dump = tag(({ key, value, show, showChange, showKids = false, showLevels = -1, 
// showAll,
format = 'small', formatChange = () => undefined, isRootDump = true, onHeaderClick = () => undefined }) => {
    if (!showChange) {
    ...
    const showChangeValue = showChange;
    ...
    let arrayView = state(undefined)(x => [arrayView, arrayView = x]);
    onInit(() => {
    ...
    function objectTemplate() {
      ...
      ${isArray ? html `
          ...
          <div style="border:1px solid black;border-radius:5px" class="bg-danger flex-stacked text-dark">
            ...
            formatChange,
            array: value, arrayView: arrayView
        })}
              </div>
            `}
          </div>
...
const arrayTable = tag(({ array, showLevels, showAll, showKids, toggleColumnDialog, columnNames, formatChange, }) => {
...
const arraysDisplay = tag(({ showLevels, showAll, showKids, array, arrayView, formatChange, }) => {
...
*/
const dumpObject = tag(({ key, showKids, show, showChange, showLevels, value, showAll, onHeaderClick, formatChange, }) => {
