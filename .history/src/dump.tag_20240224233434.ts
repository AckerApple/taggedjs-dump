...
export const dump = tag(({
  ...
  if(!showChange) {
    ...
    watch([show], (currentValue, previousValue) => {
...
const dumpArray = ({
  ...
  let show = state(false)(x => [show, show = x])
  ...
  >
    ...
    >
      <a style="flex-grow:1" onclick=${() => {
        show = !show
        // showChangeValue(show = !show);
        console.log('click 485', show)
      }}>
        <strong>${key}</strong>
      </a>
...
const dumpObject = tag(({
  ...
  let showLower = state(false)(x => [showLower, showLower = x])

  console.log('showLower',{showLower, key, value})
      ...
      >
        ${key && html`
          <a
            ...
            onclick=${() => {
              showLower = !showLower
              // showChange(showLower = !showLower);
              console.log('click', showLower);/*onHeaderClick()*/
            }}
          >
            <strong>${key}</strong>
            <sup style="opacity:80%;font-size:75%;padding-left:0.4em">
