  ...
  function objectTemplate(): Tag {
    ...
    return html`  
          ...
          <div style="position:relative;">
            <div style="display:flex;font-size:50%;position:absolute;top:-18px;right:-6px">
              ${!format || format==='small' && html`
                <a
                  style="margin:1px;border-radius:5px;color:white;"
                  class=${'flex-valign-center pad-h-xxs hover-bg-balanced ' + (showAll ? 'bg-balanced' : 'bg-dark')}
              ...
              <a
                style="margin:1px;border-radius:5px;color:white;"
                class=${'flex-valign-center pad-h-xxs hover-bg-balanced ' + (!format || format==='small' ? 'bg-positive' : 'bg-dark')}
                onclick=${() => formatChange(format='small')}
              >small</a>
              <a style="margin:1px;border-radius:5px;color:white;"
                class=${"flex-valign-center pad-h-xxs hover-bg-balanced " + (format==='json' ? 'bg-positive' : 'bg-dark')}
                onclick=${() => formatChange(format='json')}
              >json</a>
              <a style="margin:1px;border-radius:5px;color:white;" class=${"flex-valign-center pad-h-xxs hover-bg-balanced active-bg-energized " + (format==='json' ? 'bg-positive' : 'bg-dark')}
      ...
      ${isArray ? html`
          ...
          >
            <div class="pad-xxs flex-1 flex-apart"
              style=${
                "font-size:65%;border-color:white;color:white;background-color:#ef473a;" +
                show ? 'border-bottom-width:1px;border-bottom-style:solid;border-color:black;':''
              }
            >
              <a style="flex-grow:1" onclick=${() => {showChangeValue(show = !show);console.log('click 485', show)}}>
                <strong>${key}</strong>
              </a>
              <sup style="opacity:80%;font-size:75%;" class="pad-left-xs">
                <a onclick=${() => (arrayView = arrayView === 'table' ? undefined : 'table')}>table</a>
              </sup>
              <sup style="opacity:80%;font-size:75%;" class="pad-left-xs">[${value.length}]</sup>
            </div>
...
const dumpObject = tag(({
  ...
  return html`
    <div style="flex: 1 1 10em;">
      ...
      >
        ${key && html`
          <a style=${"font-size:65%;color:white;border-color:white;flex-grow:1;background-color:#387ef5;" + showLower ? 'border-bottom-width:1px;border-bottom-style:solid;border-color:black;' : ''}
            class="pad-xxs flex-apart"
